...
module.exports = (function() {
  ...
  mod.initStat = function(key, config, options, cb) {

    // (config corresponds to KEY.INFO in munin plugin 'config' output)
    if (_.isNull(config) || _.isUndefined(config)) config = {};
    _.defaults(config, {
      'type' : 'GAUGE',
      'label': key,
      'min': 0
    ...
    if (_.isNull(options) || _.isUndefined(options)) options = {};
    _.defaults(options, {
      initialVal: 0,
      resetOnDump: true,
      resetTo: 0
  ...
  mod.dumpMuninStats = function(graphKey, reset) {
      if (_.isNull(graphKey) || _.isUndefined(graphKey)) {
          return _.map(graphs, function(graphKey) {
              return mod.dumpMuninStat(graphKey, reset);
          });
      } else {
          return mod.dumpMuninStat(graphKey, reset);
      }
  ...
  mod.dumpMuninStat = function(graphKey, reset) {
